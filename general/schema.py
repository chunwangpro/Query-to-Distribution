...
class Predicate():
    ...
    def apply(self, vals):
        ...
        print ('begin apply a predicate: ', self.col_name, self.right_val, self.op, self.func(vals))
...
class Query():
    ...
    def get_left_func(self, root):
        if type(root) is psqlparse.nodes.parsenodes.ColumnRef:
            f = lambda x: x
            return f, root.fields[-1].val
        elif type(root) is psqlparse.nodes.parsenodes.AExpr:
            col_name = root.lexpr.fields[-1].val
    ...
    def extract_distinct_vals(self, queries):
        ...
        for col in self.columns:
            ...
            if col.name in distinct_values_equal:
                ...
                if len(self.distinct_vals[col.name]) > 0:
