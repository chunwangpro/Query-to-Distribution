...
class ProgressiveSampling(CardEst):
    ...
    def __init__(
        ...
        if r <= 1.0:
            ...
            self.num_samples = r
        self.seed = seed
        self.cardinality = schema.cardinality
    ...
    def sample_for_generation(self,
                  num_samples,
                  ordering,
                  inp=None):
    ...
    def _sample_n(self,
        ...
        #         column_idx = self.schema.rev_index[natural_idx]
        #         col_name = self.schema.columns[column_idx].name
        #         if col_name not in query.filter_cols and natural_idx != ncols - 1:
        #             inp = this_model.EncodeInput(
        #                 None,
        #                 natural_col=natural_idx,
        #                 out=inp)
        ...
        for i in range(ncols):
            ...
            probs_i = torch.softmax(
                this_model.logits_for_col(natural_idx, logits), 1)
            if col_name in query.filter_cols:
                valid_i = []
                ...
                # print ('Calculating Valid Completed!')
                # print ('probs_i: {}, valid_i: {}'.format(probs_i.shape, valid_i.shape))
            ...
            if i < ncols - 1:
                ...
                if i != 0:
                    ...
                    num_i = num_samples
                # if (self.shortcircuit and col_name not in query.filter_cols) or not do_sample:
                #     data_to_encode = None
                #     sampled_cols.append(None)
                # else:
                ...
                # print (col_name, probs_i.sum(1, keepdim=True).shape, probs_i.sum(1, keepdim=True)[0], probs_i[0].max(), probs_i[0].min())
                ...
                next_column_idx = self.schema.rev_index[next_natural_idx] 
                next_col_name = self.schema.columns[next_column_idx].name
                # if self.shortcircuit and next_col_name not in query.filter_cols:
