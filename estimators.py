...
class CardEst(object):
    ...
    def get_stats(self):
        return [self.query_starts, self.query_dur_ms, self.errs, self.est_cards, self.true_cards]
...
class ProgressiveSampling(CardEst):
    ...
    def _sample_n(self, num_samples, ordering, columns, operators, vals, inp=None, is_train=False):
            ...
            if op is not None:
                ...
                # split col
                    ...
                    for j in range(len(op)):
                        ...
                        val1, val2 = self.split_val(
                            index, self.table.split_col_bit[natural_idx + 1]
                        )
        ...
        if self.shortcircuit:
            for i in range(ncols):
                ...
                if operators[column_idx] is None and natural_idx != ncols - 1:
                    inp = this_model.EncodeInput(None, natural_col=natural_idx, out=inp)
        ...
        for i in range(ncols):
            ...
            if sub_col[natural_idx]:
                border_exist, valid_i = self.findborderindex(
                    samples_i, sub_col_border[natural_idx], valid_i_list[i]
                )
                if border_exist:
            ...
            if i < ncols - 1:
                ...
                samples_i = torch.multinomial(y_i, num_samples=num_i, replacement=True).view(
                    -1, 1
                )  # [bs, num_i]
    ...
    def Query(self, columns, operators, vals):
        ...
        for val in vals:
            if len(val) == 2 and val[0] > val[1]:
                return 0
        columns, operators, vals = FillInUnqueriedColumns(self.table, columns, operators, vals)
        ...
        if num_orderings == 1:
            ...
            p = self._sample_n(
                self.num_samples, inv_ordering, columns, operators, vals, inp=inp_buf, is_train=True
            )
            self.OnEnd()
            # print ('here', p)
            return p
        ...
        for ordering in orderings:
            p_scalar = self._sample_n(
                self.num_samples // num_orderings, ordering, columns, operators, vals
            )
            ps.append(p_scalar)
        self.OnEnd()
        return np.mean(ps)
