...
Original file is located at
    https://colab.research.google.com/drive/1lCCNfFuuSevE_JoqyuHZEBJipa4S8x5C
"""
...
def GenerateQuery(table, min_num_filters, max_num_filters, rng, dataset):
    ...
    num_filters = rng.randint(max_num_filters - 1,
                              max_num_filters)  ######### 这里可以改
...
def SampleTupleThenRandom(table, num_filters, rng, dataset):
    ...
    idxs = rng.choice(len(table.columns), replace=False,
                      size=num_filters)  ######## 这里可以改
...
def dictionary_column_interval(table_size, query_set):
    ...
    for k, v in column_interval.items():
        ...
        column_interval[k] = sorted([0, least / 2] + column_interval[k] +
                                    [great + 1])
...
def process_train_data(unique_intervals, query_set, train_size=1):
    ...
    df_train = pd.DataFrame(
        total, columns=[f'col_{i}' for i in range(total.shape[1])])
    # boundary
...
def generate_data_new(grid, model):
    ...
    line = pd.DataFrame(np.concatenate([grid, newpred], axis=1),
                        columns=[f'col_{i}'
                                 for i in range(n_column)] + ['card'])
    ...
    dataNew = pd.DataFrame(columns=[f'col_{i}' for i in range(n_column)],
                           index=range(n_row))
    count = 0
...
class CopulaModel(tf.keras.Model):
    def __init__(self, modelpath, dim, lattice_size=2, pwl_keypoints=None):
        ...
        self.copula_lattice = LatticeCDFLayer(dim=dim,
        ...
        for i in range(self.dim):
            self.calibrators.append(
                tfl.layers.PWLCalibration(
                    ...
                    name='col_%s_inf_pwl' % i)(self.model_inputs[2 * i]))
            self.calibrators.append(
                tfl.layers.PWLCalibration(
                    input_keypoints=np.array(
                        self.pwl_calibration_input_keypoints[i]),
                    ...
                    name='col_%s_sup_pwl' % i)(self.model_inputs[2 * i + 1]))
        ...
        self.model = tf.keras.models.Model(inputs=self.model_inputs,
                                           outputs=self.copula_lattice(
                                               self.lattice_cdf))
...
if __name__ == '__main__':
    ...
    print("\nColumn intervals", column_interval_number,
          np.product(column_interval_number))
    ...
    lhs_sample = lb + (ub - lb) * lhs(
        n_column, samples=lhs_n, criterion='center')
