...
class DataGenerator:
    ...
    def _sample_n(self,
        ...
        for i in range(ncols):
            ...
            # If wildcard enabled, 'logits' wasn't assigned last iter.
            probs_i = torch.softmax(
                this_model.logits_for_col(natural_idx, logits), 1) #该行不同取值的概率
            # Num samples to draw for column i.
            if i != 0:
                num_i = 1
            else:
            ...
            samples_i = torch.multinomial(
                probs_i, num_samples=num_i,
                replacement=True).view(-1, 1)  # [bs, num_i] #采样
    ...
    def generate_data(self, path=None, path_naru=None):
        ...
        self.model.eval()
        self.kZeros = torch.zeros(self.table.cardinality,
                                    self.model.nin,
                                    device=self.DEVICE)
        self.inp = self.model.EncodeInput(self.kZeros)
        self.inp = self.inp.view(self.table.cardinality, -1)
        return self._sample_n(self.table.cardinality,
                    self.model,
                    self.table,
                    inp=self.inp,)
