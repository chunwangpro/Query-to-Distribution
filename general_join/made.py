...
class MADE(nn.Module):
    ...
    def __init__(
        ...
        encoded_bins = [self._get_output_encoded_dist_size(self.input_bins[i],i) for i in range(self.nin)]
        
        self.input_bins_encoded = [self._get_input_encoded_dist_size(self.input_bins[i],i) for i in range(self.nin)]
        ...
        if self.input_encoding == 'embed':
            ...
            for i, dist_size in enumerate(self.input_bins):
                if  type(pca_embed[i]) is not np.ndarray:
                    if dist_size <= self.embed_size and self.input_no_emb_if_leq:
                        embed = None
                    else:
                        embed = nn.Embedding(dist_size, self.embed_size)
                else:
                    #embed = torch.tensor(pca_embed[i], requires_grad=False)
                    embed = nn.Embedding(dist_size, self.embed_size)
                    embed.weight.data.copy_(torch.from_numpy(pca_embed[i]))
    ...
    def _get_input_encoded_dist_size(self, dist_size,col):
        if self.input_encoding == 'embed':
        ...
        elif self.input_encoding == None :
            return 1
        else:
            assert False, self.input_encoding
        return dist_size

    def _get_output_encoded_dist_size(self, dist_size,col):
        if self.output_encoding == 'embed':
            if self.input_no_emb_if_leq:
                dist_size = min(dist_size, self.embed_size)
            else:
    ...
    def Embed(self, data, natural_col=None, out=None):
        if data is None:
            ...
            # print (out.shape, self.unk_embeddings[natural_col].shape)
            out = out.clone().detach()
            if natural_col == 0:
                out[:, :self.input_bins_encoded_cumsum[0]] = out[:, :self.input_bins_encoded_cumsum[0]] + self.unk_embeddings[natural_col]
            else:
                l = self.input_bins_encoded_cumsum[natural_col - 1]
                r = self.input_bins_encoded_cumsum[natural_col]
                out[:, l:r] = out[:, l:r] + self.unk_embeddings[natural_col]
            # print (out.shape)
            return out
        ...
        if natural_col is not None:
            ...
            # Embed?
            # print (natural_col, coli_dom_size, self.embed_size, self.input_no_emb_if_leq)
            if  coli_dom_size > self.embed_size or not self.input_no_emb_if_leq:
                # print ('data', data.shape[0], data[0:10])
                all_embeddings = self.embeddings[natural_col]
                # (torch.cuda.LongTensor([i for i in range(data.shape[1])]))
                # print ('data: ', data.shape, 'all_embedding: ', all_embeddings.shape)
                res = torch.matmul(data, all_embeddings.weight)
                # print ('data: ', data.requires_grad, data.grad_fn)
                # print ('res', res.requires_grad, res.grad_fn)
                if out is not None:
                    # out.copy_(res)
                    out = out.clone().detach()
                    if natural_col == 0:
                        out[:, :self.input_bins_encoded_cumsum[0]] = out[:, :self.input_bins_encoded_cumsum[0]] + res
                    else:
                        l = self.input_bins_encoded_cumsum[natural_col - 1]
                        r = self.input_bins_encoded_cumsum[natural_col]
                        out[:, l:r] = out[:, l:r] + res
                    # print ('out', out.requires_grad, out.grad_fn)
                    return out
                return res
            else:
                if out is None:
                ...
                out = out.clone().detach()
                if natural_col == 0:
                    out[:, :self.input_bins_encoded_cumsum[0]] = out[:, :self.input_bins_encoded_cumsum[0]] + data
                else:
                    l = self.input_bins_encoded_cumsum[natural_col - 1]
                    r = self.input_bins_encoded_cumsum[natural_col]
                    out[:, l:r] = out[:, l:r] + data
    ...
    def do_PCA(self,data,natural_col=None, out=None):
        ...
        if natural_col is not None:
            ...
            emb = torch.as_tensor(self.pca_emb[natural_col]).to(data.device).to(torch.float32)
            res = emb[data.view(-1,)]
            ...
            for i in range(len(self.pca_emb)):
                emb = torch.as_tensor(self.pca_emb[i]).to(data.device).to(torch.float32)
                col_i_embs = emb[data[:,i].long()]
                if not self.column_masking:
                    y_embed.append(col_i_embs)
                else:
                    dropped_repr = self.unk_embeddings[i]
    ...
    def forward_with_encoded_input(self, x):
        #if self.input_encoding == None:
        #    x=  F.normalize(x, p=2, dim=1)
        # print ('forward_with_encoded_input')
        if self.direct_io_layer is not None:
            residual = self.direct_io_layer(x)
            return self.net(x) + residual
    ...
    def logits_for_col(self, idx, logits):
        ...
        # return logits
        if idx == 0:
            # print ('get column ', 0, self.logit_indices[0])
            logits_for_var = logits[:, :self.logit_indices[0]]
        else:
            # print ('get column ', self.logit_indices[idx - 1], self.logit_indices[idx])
            logits_for_var = logits[:, self.logit_indices[idx - 1]:self.
                                    logit_indices[idx]]
        # print ('logits_for_var: ', logits_for_var.shape)
        if self.output_encoding != 'embed':
        ...
        probs = torch.matmul(logits_for_var, embed.weight.t())
    ...
    def nll(self, logits, data):
        ...
        for i in range(self.nin):
            ...
            # print (logits_i.shape, data[:, i].shape)
            prediction = torch.argmax(logits_i, 1)
           
            nll += F.cross_entropy(logits_i, data[:, i], reduction='none')
            # print (F.cross_entropy(logits_i, data[:, i], reduction='none'))
            acc.append((prediction == data[:, i]).sum().float()/bs)
