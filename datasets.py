        ...
        for column in sql[table]:
            ...
            if tmp == '':
                ...
                if len(tmp) == 4:
                    ...
                    op_col = [tmp[0].strip(' ')]
                    op_col,val_col = convert_op(op_col,[convert_type(tmp[1].strip(' '),column_type)])
                    op.append(op_col)
                    val.append(val_col)
                else:
...
def LoadTitle(filename='title.csv'):
    csv_file = '../datasets/{}'.format(filename)
    cols = [
        'kind_id', 'production_year', 'season_nr'
    ]
    type_casts = {'production_year': str, 'season_nr': str}
    # Note: other columns are converted to objects/strings automatically.  We
    # don't need to specify a type-cast for those because the desired order
    # there is the same as the default str-ordering (lexicographical).
    return common.CsvTable('title', csv_file, cols, type_casts, low_memory=False, quoting=csv.QUOTE_ALL, escapechar='\\')
...
def LoadMovieInfo(filename='movie_info.csv'):
    ...
    # don't need to specify a type-cast for those because the desired order
    # there is the same as the default str-ordering (lexicographical).
    return common.CsvTable('movie_info', csv_file, cols, type_casts, low_memory=False, quoting=csv.QUOTE_ALL, escapechar='\\')

def LoadMovieCompany(filename='movie_companies.csv'):
    csv_file = '../datasets/{}'.format(filename)
    cols = [
        'company_id','company_type_id','note'
    ]
    type_casts = {}
    # Note: other columns are converted to objects/strings automatically.  We
    # don't need to specify a type-cast for those because the desired order
    # there is the same as the default str-ordering (lexicographical).
    return common.CsvTable('movie_companies', csv_file, cols, type_casts, low_memory=False, quoting=csv.QUOTE_ALL, escapechar='\\')
...
def LoadCastInfo(filename='cast_info.csv'):
    ...
    # don't need to specify a type-cast for those because the desired order
    # there is the same as the default str-ordering (lexicographical).
    return common.CsvTable('cast_info', csv_file, cols, type_casts, low_memory=False, quoting=csv.QUOTE_ALL, escapechar='\\')

def LoadTitleJoinCastinfo(filename1='title.csv', filename2='cast_info.csv', left_key='id', right_key='movie_id', generated_file=None):
    cols = [
        'kind_id', 'production_year', 'season_nr', 'nr_order', 'role_id'
    ]
    ...
    title_cols = [
        'id', 'kind_id', 'production_year', 'season_nr'
    ]
    cast_info_cols = [
        'movie_id', 'nr_order', 'role_id'
    ]
    title = pd.read_csv('../datasets/{}'.format(filename1), usecols=title_cols, low_memory=False, quoting=csv.QUOTE_ALL, escapechar='\\')
    cast_info = pd.read_csv('../datasets/{}'.format(filename2), usecols=cast_info_cols, low_memory=False, quoting=csv.QUOTE_ALL, escapechar='\\')
...
def LoadTitleJoinMovieinfo(filename1='title.csv', filename2='movie_info.csv', left_key='id', right_key='movie_id'):
    title_cols = [
        'id', 'kind_id', 'production_year', 'season_nr'
    ]
    movie_info_cols = [
    ...
    movie_info = pd.read_csv('../datasets/{}'.format(filename2), usecols=movie_info_cols, low_memory=False)
    joined_table = title.join(movie_info.set_index('movie_id'), on='id', how='inner')
    cols = [
        'title', 'kind_id', 'production_year', 'phonetic_code',
        'episode_of_id', 'season_nr', 'episode_nr', 'info_type_id', 'info'
    ]
    type_casts = {'production_year': str, 'season_nr': str}
    return common.CsvTable('title_movie_info', joined_table, cols, type_casts)

def LoadTitleJoinMoviecompany(filename1='title.csv', filename2='movie_companies.csv', left_key='id', right_key='movie_id'):
    title_cols = [
        'id', 'kind_id', 'production_year', 'season_nr'
    ]
    movie_company_cols = [
    ...
    title = pd.read_csv('../datasets/{}'.format(filename1), usecols=title_cols, low_memory=False)
    movie_company = pd.read_csv('../datasets/{}'.format(filename2), usecols=movie_company_cols, low_memory=False)
    joined_table = title.join(movie_company.set_index('movie_id'), on='id', how='inner')
    cols = [
        'kind_id', 'production_year', 'season_nr', 'company_id','company_type_id','note'
    ]
    type_casts = {'production_year': str, 'season_nr': str}
...
def LoadDataset(filename,dataset_name,cols = None,type_casts = {},drop_col=[],na_values={}):   
    ...
    csv_file = 'datasets/{}'.format(filename)
    df = pd.read_csv(csv_file,header = None,na_values=na_values,quotechar='"', delimiter=',',
