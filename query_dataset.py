...
parser.add_argument('--test-query-num',
                    type=int,
                    default=100,
                    help='num of tested queries')
parser.add_argument('--test-num-conditions',
                    type=int,
                    default=5,
                    help='num of conditions in each tested query')
...
def GenerateQuery(table_origin, min_num_filters, max_num_filters, rng, dataset,
                  table_generate):
    """Generate a random query."""
    num_filters = rng.randint(max_num_filters - 1, max_num_filters)

    cols, idxs, ops, vals = SampleTupleThenRandom(table_origin, num_filters,
                                                  rng, dataset)
    # print (vals)
...
if __name__ == '__main__':
    ...
    if args.dataset == 'dmv-tiny':
        ...
        table_origin = datasets.LoadDataset(args.dataset + '.csv',
                                            args.dataset,
                                            type_casts=type_casts)
        table_generate = datasets.LoadDataset('generated_' + args.dataset +
                                              '_random.csv',
    ...
    for i in range(args.test_query_num):
        cols, idxs, ops, vals, sel, sel2 = GenerateQuery(
            table_origin, 2, args.test_num_conditions + 1, rng, args.dataset,
            table_generate)
        ...
        # print (sel, sel2)
    print(np.mean(diff), np.median(diff), np.percentile(diff, 90),
          np.percentile(diff, 95), np.percentile(diff, 99), np.max(diff))
