...
class DataGenerator:
    ...
    def resample(generated_hist, origin_hist, origin_meta_table, generated_table):
        target_card = origin_meta_table.cardinality
        current_card = len(generated_table)
        appended_data = []
        col = 0
        for o_hist, g_hist in zip(origin_hist, generated_hist):
            for idx, row in g_hist:
                cnt = row[0]
                current_cnt = g_hist[idx][0]
                o_hist[idx][0] = max(0, cnt - current_cnt)
            vals = o_hist.index.values
            probs = o_hist.iloc[:, 0].values
            sample_idx = np.random.choice(len(vals), size=target_card-current_card, replace=True, p=probs)
            append_col = []
            append_col += vals[sample_idx]
            append_col += generated_table.iloc[:, col].values
            appended_data.append(append_col)
            col += 1
...
if __name__ == '__main__':
    ...
    idx_mapping_join2title = [{} for _ in title_table.columns]
    for k, c in enumerate(title_table.columns):
        # print (c.all_distinct_values)
        # print (joined_table.columns[k].all_distinct_values)
        distinct = c.all_distinct_values.tolist()
        for idx, v in enumerate(joined_table.columns[k].all_distinct_values):
            try:
                idx_j = distinct.index(v)
                idx_mapping_join2title[k][idx] = idx_j
            except:
                pass

    print (idx_mapping_join2title)
    ...
    generated_data_join, generated_data_join_idx = model.generate_data(scale=args.scale, path='title_cast_info')
    print (len(generated_data_join))

    generated_data_title_idx = generated_data_join_idx[0:3]
    for k in range(len(generated_data_title_idx)):
        for kk in range(len(generated_data_title_idx[k])):
            ...
            if key in idx_mapping_join2title[k]:
                generated_data_title_idx[k][kk][0] = idx_mapping_join2title[k][key]
            else:
                generated_data_title_idx[k][kk][0] = -1

    generated_title_idx_df = pd.DataFrame(data={'kind_id':[title_table.columns[0].all_distinct_values[i] for i in generated_data_title_idx[0]],'production_year':[title_table.columns[1].all_distinct_values[i] for i in generated_data_title_idx[1]], 'season_nr':[title_table.columns[2].all_distinct_values[i] for i in generated_data_title_idx[2]]})
    generated_title_idx_df.drop_duplicates(subset=None, keep='first', inplace=True)

    generated_title_hist_idx_kind_id = generated_title_idx_df.groupby('kind_id').count()
    print (generated_title_hist_idx_kind_id.head(5))
    generated_title_hist_idx_production_year = generated_title_idx_df.groupby('production_year').count()
    generated_title_hist_idx_season_nr = generated_title_idx_df.groupby('season_nr').count()

    origin_title_hist_kind_id = title_table.data.groupby('kind_id').count()
    origin_title_hist_production_year = title_table.data.groupby('production_year').count()
    origin_title_hist_season_nr = title_table.data.groupby('season_nr').count()

    generated_hist = (generated_title_hist_idx_kind_id, generated_title_hist_idx_production_year, generated_title_hist_idx_season_nr)
    origin_hist = (origin_title_hist_kind_id, origin_title_hist_production_year, origin_title_hist_season_nr)

    generated_data_title = model.resample(path='title', generated_hist, origin_hist, title_table, generated_title_idx_df)
